...
def pipeline(spark: SparkSession) -> None:
    ...
    df_AdjustCols = AdjustCols(spark, df_JoinedShippingData)
    df_AdjustCols = df_AdjustCols.cache()
    df_ForFollowUp = ForFollowUp(spark, df_AdjustCols)
    df_ForFollowUp = df_ForFollowUp.cache()
    df_SumAmounts = SumAmounts(spark, df_ForFollowUp)
    ...
    df_SumAmounts_1 = SumAmounts_1(spark, df_ForFollowUp)
